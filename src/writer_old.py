...
from .config import MAX_FILE_SIZE_BYTES, ENV_VARS
...
class JsonlWriter:
        ...
        try:
            for record in records:
                ...
                if not final_record["document_id"]:
                    logger.warning("Skipping record without 'carr_eqp_uid' (document_id).")
                    continue
    ...
    def upload_files(self, conn_str: str, container_name: str):
        ...
        """
        logger.info(f"Uploading {len(self.generated_files)} files to container '{container_name}'...")
        try:
