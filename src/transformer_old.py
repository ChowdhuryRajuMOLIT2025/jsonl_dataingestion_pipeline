...
from .config import (BOOLEAN_COLUMNS, COLUMN_MAPPING, DATE_COLUMNS,
                     DROP_COLUMNS_AFTER_LOAD, ID_COLUMNS,
...
class DataTransformer:
    ...
    def normalize_and_rename_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_targets:
            logger.warning(
                f"Missing mapped columns in source, initializing as empty: {missing_targets[:10]}..."
            )
    ...
    def clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(obj_cols) > 0:
            ...
            df[obj_cols] = df[obj_cols].replace(
                {"nan": pd.NA, "NaN": pd.NA, "None": pd.NA, "null": pd.NA, "": pd.NA}
            )
    ...
    def split_multivalued_ids(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def _split_to_list(val):
            if val is None or pd.isna(val):
                return (
                    []
                )  # Return empty list for consistency? Or None? Azure Search handles [] well.
            s = str(val).strip()
    ...
    def enrich_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["critical_dates_summary"] = df.apply(
            self._derive_critical_dates_summary, axis=1
        )
    ...
    def derive_optimal_dates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        derived = pd.to_datetime(
            df.get("derived_ata_dp_date"), errors="coerce", utc=True
        )
        ...
        predictive = pd.to_datetime(
            df.get("predictive_eta_fd_date"), errors="coerce", utc=True
        )
        revised = pd.to_datetime(
            df.get("revised_eta_fd_date"), errors="coerce", utc=True
        )
    ...
    def derive_shipment_status(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        empty_ret = pd.to_datetime(
            out.get("empty_container_return_date"), errors="coerce", utc=True
        )
        delivery = pd.to_datetime(
            out.get("delivery_to_consignee_date"), errors="coerce", utc=True
        )
        out_dp = pd.to_datetime(
            out.get("out_gate_from_dp_date"), errors="coerce", utc=True
        )
        equip_arr = pd.to_datetime(
            out.get("equipment_arrived_at_last_cy_date"), errors="coerce", utc=True
        )
        out_last_cy = pd.to_datetime(
            out.get("out_gate_at_last_cy_date"), errors="coerce", utc=True
        )

        dp_opt = pd.to_datetime(
            out.get("optimal_ata_dp_date"), errors="coerce", utc=True
        )
        dp_derived = pd.to_datetime(
            out.get("derived_ata_dp_date"), errors="coerce", utc=True
        )
        ...
        m_last_cy = (
            (equip_arr.notna() | out_last_cy.notna())
            & ~m_empty
            & ~m_delivered
            & ~m_in_inland
        )
    ...
    def derive_unified_delays(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        del_ts = pd.to_datetime(
            df.get("delivery_to_consignee_date"), errors="coerce", utc=True
        )
        ret_ts = pd.to_datetime(
            df.get("empty_container_return_date"), errors="coerce", utc=True
        )
    ...
    def _safe_str(val) -> Optional[str]:
        if (
            pd.isna(val)
            or val is None
            or str(val).lower() in {"nan", "none", "null", "<na>", ""}
        ):
            return None
    ...
    def _derive_vessel_summary(self, row) -> Optional[str]:
        ...
        parts = []
        if f:
            parts.append(f"First vessel: {f}")
        if l and l != f:
            parts.append(f"Final vessel: {l}")
        return " | ".join(parts)
    ...
    def _derive_carrier_summary(self, row) -> Optional[str]:
        ...
        if fc and scac and fc != scac:
            return f"Operated by {fc} ({scac})"
        if fc:
            return f"Operated by {fc}"
        if scac:
            return f"Operated by {scac}"
        return None
    ...
    def _derive_port_route_summary(self, row) -> Optional[str]:
        ...
        parts = []
        # Origin
        if por and lp and por != lp:
            parts.append(f"Origin: {por} -> Load Port: {lp}")
        elif lp:
            parts.append(f"Origin: {lp}")
        elif por:
            parts.append(f"Origin: {por}")

        # TS
        if flp and flp not in {lp, dp}:
            parts.append(f"Transshipment Port: {flp}")

        # DP
        if dp:
            parts.append(f"Discharge Port: {dp}")

        # POD
        if pod and pod != dp:
            parts.append(f"Delivery At: {pod}")

        # FD
        if fd:
            if pod and fd != pod:
                parts.append(f"Final: {fd}")
            elif not pod and fd != dp:
                parts.append(f"Final: {fd}")
    ...
    def _derive_critical_dates_summary(self, row) -> str:
        ...
        last_cy = (
            self._safe_str(row.get("equipment_arrived_at_last_cy_lcn")) or "Last CY"
        )
        ...
        def _delay_str(stat, d):
            ...
            val = int(d)
            if stat == "delay":
                return f"delayed by {val}d"
            if stat == "early":
                return f"early by {abs(val)}d"
        ...
        if etd_lp and atd_lp:
            ...
            comment = (
                f"departed {d}d after ETD"
                if d > 0
                else (f"departed {abs(d)}d early" if d < 0 else "departed on-time")
            )
        elif etd_lp:
        ...
        legs.append(
            f"Stage 1 (POR->LP): {por}->{lp} (ETD={etd_lp}, ATD={atd_lp}; {comment})"
        )

        # Leg 2: LP -> TS
        if ts:
            c2 = "no events"
            if ata_flp or atd_flp:
                c2 = f"Arr={ata_flp}, Dep={atd_flp}"
        ...
        legs.append(
            f"Stage 3 (Ocean->DP): To {dp} (ETA={eta_dp}, ATA={ata_dp}, {dp_d_s})"
        )

        # Leg 4: DP -> CY
        c4 = "no info"
        if out_dp_dt and arr_cy:
            c4 = f"Dep DP {out_dp_dt}, Arr CY {arr_cy}"
        elif out_dp_dt:
            c4 = f"Dep DP {out_dp_dt}"
        elif arr_cy:
            c4 = f"Arr CY {arr_cy}"
        ...
        c5 = (
            f"Completed {fin_dt}"
            if fin_dt
            else ("Pending" if eta_fd and eta_fd >= today else "Overdue/Unknown")
        )
        legs.append(
            f"Stage 5 (CY->Final): {out_cy_lcn}->{fd} (ETA={eta_fd}, {c5}, {fd_d_s})"
        )
    ...
    def _derive_delay_reason_and_gaps(self, row) -> pd.Series:
        ...
        if dp_dur and dp_dur > 0:
            ...
            if atd_lp and etd_lp and (atd_lp - etd_lp).days > 1:
                reasons.append(
                    f"Origin departure delay of {(atd_lp - etd_lp).days} days contributed."
                )
        ...
        # Gaps
        if not atd_lp and (ata_dp or out_dp):
            gaps.append("missing_atd_lp")
        if (eta_dp or ata_dp) and not out_dp and (arr_cy or del_dt):
            gaps.append("missing_out_gate_from_dp")
        if not eta_fd and (del_dt or emp_rt):
